...
public class AIWeatherService {
    ...
    public void checkAndHandleForecasts() {
        ...
        for (WeatherForecast forecast : forecasts) {
            ...
            // ------------------------------------------------------
            // ORIGINAL CODE (causing error because aiHandled is a String):
            // if (forecast.getAiHandled() == null || !forecast.getAiHandled()) {
            //
            // NEW CODE (string-based check):
            // We treat "true" as the AI-handled state, everything else as not handled.
            // ------------------------------------------------------
            // if (forecast.getAiHandled() == null || !forecast.getAiHandled()) {
            if (forecast.getAiHandled() == null || !"true".equalsIgnoreCase(forecast.getAiHandled())) {
                ...
                // ----------------------------------------------------
                // ORIGINAL CODE (causing error because setAiHandled(...) expects a String):
                // forecast.setAiHandled(true);
                //
                // NEW CODE (string-based assignment):
                // ----------------------------------------------------
                // forecast.setAiHandled(true);
                forecast.setAiHandled("true");
    ...
    private String buildEmailHtml(WeatherForecast forecast, Long businessId) {
        ...
        return "<html><body>"
                ...
                + "<p>"
                + "<a href='http://3.95.195.91:8080/api/v1/ai-weather/handle-yes?forecastId=" + forecast.getForecastId() 
                    + "&businessId=" + businessId + "'>YES, handle</a><br>"
                + "<a href='http://3.95.195.91:8080/api/v1/ai-weather/handle-no?forecastId=" + forecast.getForecastId() 
